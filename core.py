...
class Compare(BaseCompare):
    ...
    def _intersect_compare(self, ignore_spaces: bool, ignore_case: bool) -> None:
        ...
        """
        inequality_flag=1
        self.compare_status=f''' <b style="background-color:#00FF00">Match</b> '''
        ...
        for column in self.intersect_columns():
            ...
            self.column_stats_report_1.append([column,row_cnt,match_cnt,row_cnt-match_cnt])
            if row_cnt-match_cnt>0 and inequality_flag==1:
                inequality_flag=2
                self.compare_status=f'''<b style="background-color:#FF0000"> Mismatch</b> '''
            elif row_cnt==0 and inequality_flag==1:
                inequality_flag=2
                self.compare_status=f'''<b style="background-color:#FF0000"> No Comparision due to no Common Rows</b> '''
    ...
    def report(
        ...
        new_html_report+=render("html_template.html",
                                self.df1_name,
                                self.df2_name,
                                match_on,
                                "Yes" if self._any_dupes else "No",
        ...
        report1=""
        report1+="<table border='1px'"
        report1+="\n\n"
        report1+='<tr><th>Source File </th><td>'+self.df1_name+'</td></tr>'
        report1+='<tr><th>Target File </th><td>'+self.df2_name+'</td></tr>'
        report1+='<table>'
        report1+="\n\n"
        report1+="<table border='1px'>"
        report1+="\n\n"
        report1+='<tr><th>Column Name</th><th>Total Rows</th><th>Matched Rows</th><th>Mismatched Rows</th>'

       
        if html_file:

            this_dir=os.path.dirname(os.path.realpath(__file__))
            with open(os.path.join(this_dir,"templates","style.css")) as file_open:
                html_file_output+=file_open.read()
            html_file_output+=new_html_report

            if len(self.df1_unq_columns())>0:
                html_file_output+=''' <td class="sub-table=td"><br><br><table class="table4" id="tab4">
                <thead><tr><th>Columns only in Source file </th></tr></thead> '''
            
                for i in self.df1_unq_columns():
                    html_file_output+='<tr><td>'+str(i)+'</td></tr>'
                html_file_output+="</table></td>"

            if len(self.df2_unq_columns())>0:
                html_file_output+=''' <td class="sub-table=td"><br><br><table class="table4" id="tab4">
                <thead><tr><th>Columns only in Target file </th></tr></thead> '''
            
                for i in self.df2_unq_columns():
                    html_file_output+='<tr><td>'+str(i)+'</td></tr>'
                html_file_output+="</table></td></tr></table>"

            tmp_stlye_txt='''</p></div><div class="tabs__tab" id="tab_2" data-tab-info><p><button class="button1"   onclick=display_all()>All Columns </button>
            <button class="button1" onclick=display_only_mismatch()>Display Mismatch Columns</button>
            <button class="button1" onclick=display_only_match()>Display Match Columns</button>
            <br><br>
            <table class="table4" id="tab4_1"><thead><tr><th>Column Name</th><th>Total Rows</th><th>Matched Rows</th>
            <th>Mismatched Rows</th></tr></thead></head>'''     
        
            html_file_output+=tmp_stlye_txt
            for i in self.column_stats_report_1:
                if int(i[3])>0:
                    html_file_output+='<tr style="background-color:#FF0000"><td>'+str(i[0].upper())+'</td><td>'+str(i[1])+'</td><td>'+str(i[2])+'</td><td>'+str(i[3])+'</td><tr>'
            
                elif int(i[3])==0:
                    html_file_output+='<tr style="background-color:#00FF00"><td>'+str(i[0].upper())+'</td><td>'+str(i[1])+'</td><td>'+str(i[2])+'</td><td>'+str(i[3])+'</td><tr>'
            html_file_output+="</table></p></div>"
            html_report=report.replace("\n","<br>").replace(" ","&nbsp;")
            html_report=f"<pre>{html_report}</pre>"
            html_file_output+=f'''<div class="tabs__tab" id="tab_3" data-tab-info><p>'''
            html_file_output+=html_report
            html_file_output+='</p></div></html>'

            with open(os.path.join(this_dir,"templates","value_check.js")) as file_open:
                html_file_output+=file_open.read()
            with open(html_file,"w",encoding="utf-8") as f:
                f.write(html_file_output)

        return html_file_output
